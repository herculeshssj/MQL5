...
//-- include
#include <Artigos\ArtigoTrailingStop.mqh> // Include trailing stop class
...
input double   TrailingSARStep=0.02;   // Trailing SAR step
input double   TrailingSARMaximum=0.2; // Trailing SAR maximum
...
int OnInit()
  {
   ...
   trailingStop.Init(_Symbol, PERIOD_CURRENT, true, true, false); // Initialize (set basic parameters)
   if (!trailingStop.setupParameters(TrailingSARStep, TrailingSARMaximum)) {
      // Set parameters of used trailing stop type
      Alert("Error in initializing trailing stop class! Exiting...");
      return(-1);
   }
   trailingStop.startTimer(); // Start timer
   trailingStop.on();
